...
local utils = dofile(reaper.GetResourcePath() .. "/Scripts/EB Record Band/utils/EB_Utils.lua")
...
-- Get project root (parent of audio folder)
local project_root = audio_folder:match("^(.+)\\[^\\]*$") or audio_folder
local date_str = os.date("%Y-%m-%d %Hh%Mm")
local render_folder = project_root .. "\\Records\\" .. date_str

os.execute("mkdir \"" .. render_folder .. "\" 2>nul")
...
local cfillion_script = ("%s/Scripts\\ReaTeam Scripts\\Rendering\\cfillion_Apply render preset.lua"):format(reaper.GetResourcePath())
